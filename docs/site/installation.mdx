...
Upgrading from releases before 0.4 (the old profile model) requires a fresh `authsome init` and re-login. Credentials under `profile:*` keys are not migrated automatically. See [Changelog](/changelog).
