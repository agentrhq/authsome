...
1. Run `uvx authsome skill install claude` to install the bundled skill.
...
The authsome repository ships a Claude Code skill. Install it into your local Claude Code skills directory:

```bash
uvx authsome skill install claude
```

This command copies the skill manifest to `~/.claude/skills/authsome/`. It also installs a global version to `~/.skills/authsome/` which is verifiable via `npx skills list -g`.
