...
```bash
authsome get wistia --field status
# → connected
```
...
```bash Environment
eval "$(authsome export wistia --format env)"
echo $WISTIA_API_KEY
```
...
```bash
authsome login wistia --connection personal
authsome login wistia --connection team
```

## Rotate or remove the key

```bash
authsome login wistia --force          # rotate
authsome logout wistia                  # remove local credential
authsome advanced remove wistia                  # remove all local state for the provider
```
...
```bash
authsome inspect wistia > ~/.authsome/providers/wistia.json
# edit fields
authsome list   # source now shows "custom" for wistia
```
