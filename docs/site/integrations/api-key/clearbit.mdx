...
```bash
authsome get clearbit --field status
# → connected
```
...
```bash Environment
eval "$(authsome export clearbit --format env)"
...
```bash
authsome login clearbit --connection personal
authsome login clearbit --connection team
```
...
```bash
authsome inspect clearbit > ~/.authsome/providers/clearbit.json
# edit fields
