...
<Update label="0.2.4" description="2026-05-08">
  ...
  **Hosted daemon deployment.** Added `AUTHSOME_DAEMON_URL` (client-side) and `AUTHSOME_SERVER_BASE_URL` (server-side) for shared internal deployments. Approved only behind VPN or a private overlay network; the v1 daemon has no per-user authentication or tenant isolation and is not safe to expose publicly. See [Hosted deployment model](/security/hosted-deployment) and the [threat model](/security/threat-model) for supported deployment shapes.
