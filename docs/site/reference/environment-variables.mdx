...
| Variable | Purpose |
|----------|---------|
| `AUTHSOME_BASE_URL` | Runtime target and public URL. Defaults to `https://api.authsome.ai`. On client machines, set this to point the CLI and proxy at a local or self-hosted daemon. On the daemon host, set this to the public URL when behind a reverse proxy so authsome can build OAuth callback URLs such as `/auth/callback/oauth`. |
