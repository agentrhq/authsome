...
| `POST` | `/credentials/export` | Return `KEY=value` lines or JSON for a provider's connection, mirroring `authsome export`. |
...
These routes back the local HTTP proxy started by `authsome run`. The proxy never holds decryption keys itself; it asks the daemon for fresh credentials per request.
