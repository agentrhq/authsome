...
| `api_url` | No (recommended) | API host for proxy routing. Bare host, full URL, or `regex:<pattern>`. |
...
| `device_authorization_url` | If `supports_device_code` | Device authorization endpoint. Supports `{base_url}`. |
...
| `supports_device_code` | No | `true` if the device authorization grant is available. |
...
The `api_url` field controls proxy routing. See [Proxy injection](/concepts/proxy-injection) for matching rules.
...
|--------|-------------------|
| `pkce` | `oauth2` |
| `device_code` | `oauth2` (provider must set `supports_device_code: true`) |
| `dcr_pkce` | `oauth2` (provider must set `supports_dcr: true`) |
| `api_key` | `api_key` |

`authsome advanced register` validates these constraints and rejects mismatched JSON.
...
`authsome advanced register` enforces:
