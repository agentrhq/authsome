...
The top-level CLI is agent-first: `authsome --help` shows the core commands an agent is expected to learn first. Operator-oriented and human-facing workflows live under `authsome advanced ...`. Hidden legacy aliases such as `authsome rekey` and `authsome daemon status` still work for backward compatibility.
