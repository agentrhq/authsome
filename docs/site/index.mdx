...
Authsome is a local credential layer for AI agents. You authenticate once with a provider (GitHub, Google, OpenAI, Linear, and 41 other bundled services) and authsome keeps the credentials fresh for every agent run that follows.
...
Authsome acts as the local credential layer agents call at runtime.

- **No credential sprawl.** One encrypted store, every provider, every agent, one place.
- **No SaaS dependency.** Credentials never leave your machine.
...
<Frame caption="Agents call authsome at runtime. The vault decrypts and returns a fresh token; the vault never leaves the machine.">
```mermaid
flowchart LR
...
Credentials are stored locally in an encrypted SQLite vault, refreshed before expiry, and injected into agents transparently through a local proxy (`authsome run`). No server. No account. No cloud.
