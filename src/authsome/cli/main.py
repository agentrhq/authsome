...
class AgentFirstGroup(click.Group):
    """Click group that hides advanced commands from top-level help."""

    def format_commands(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        rows: list[tuple[str, str]] = []
        for subcommand in self.list_commands(ctx):
            command = self.get_command(ctx, subcommand)
            if command is None or getattr(command, "agent_hidden", False):
                continue
            rows.append((subcommand, command.get_short_help_str()))

        rows.sort(key=lambda row: (row[0] == "advanced", row[0]))
