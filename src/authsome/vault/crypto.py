...
class VaultCrypto(ABC):
    ...
    def source_id(self) -> str:
        """Stable source identifier used in CLI and API output."""
        ...

    @property
    @abstractmethod
    def source_description(self) -> str:
        """Human-readable description of where the master key came from."""
        ...
...
def _decode_master_key(encoded_value: str, source: str) -> bytes:
    ...
    try:
        master_key = base64.b64decode(encoded_value)
...
def _create_auto_crypto(key_file: Path | None) -> VaultCrypto:
    """Resolve the strongest usable master-key source without breaking existing installs."""
